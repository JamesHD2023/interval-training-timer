 ...
 */
import { createPreset as api_timer_create_preset_createPreset } from "~backend/timer/create_preset";
import { createSession as api_timer_create_session_createSession } from "~backend/timer/create_session";
import { deletePreset as api_timer_delete_preset_deletePreset } from "~backend/timer/delete_preset";
import { listPresets as api_timer_list_presets_listPresets } from "~backend/timer/list_presets";
import { listSessions as api_timer_list_sessions_listSessions } from "~backend/timer/list_sessions";
...
export namespace timer {
    ...
    export class ServiceClient {
         ...
         */
        public async createPreset(params: RequestType<typeof api_timer_create_preset_createPreset>): Promise<ResponseType<typeof api_timer_create_preset_createPreset>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/presets`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_timer_create_preset_createPreset>
        }

        /**
         * Creates a new completed session record.
         */
        public async createSession(params: RequestType<typeof api_timer_create_session_createSession>): Promise<ResponseType<typeof api_timer_create_session_createSession>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/sessions`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_timer_create_session_createSession>
        }

        /**
         * Deletes a custom timer preset.
         */
        public async deletePreset(params: { id: number }): Promise<void> {
            await this.baseClient.callTypedAPI(`/presets/${encodeURIComponent(params.id)}`, {method: "DELETE", body: undefined})
        }

        /**
         * Retrieves all saved custom timer presets.
         */
        public async listPresets(): Promise<ResponseType<typeof api_timer_list_presets_listPresets>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/presets`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_timer_list_presets_listPresets>
        }

        /**
         * Retrieves all completed sessions, ordered by completion date (latest first).
         */
        public async listSessions(): Promise<ResponseType<typeof api_timer_list_sessions_listSessions>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/sessions`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_timer_list_sessions_listSessions>
        }
    }
}
